...
We will primarily illustrate the method in the special case of K3 Kummer surfaces. Time permitting, I will also touch on degree 4 del Pezzo surfaces, which will be discussed in more detail in later lectures of Lyczak. A sizeable portion of the course will be dedicated to the study of 2- and 4-Selmer groups in quadratic twist families, which forms a large part of the technical input to some of the results referenced above.
\end{abstract}
...
Throughout, $k$ will denote a number field and $G_k$ its absolute Galois group of $k$. For a smooth projective variety $X$ over $k$ we have the algebraic Brauer group
...
\subsection{Basic strategy}
 Take the notation from Example \ref{ex:Kummer}. By a conjecture of Skorobogatov, one expects that the Brauer--Manin obstruction explains all failures of the Hasse principle for $K3$ surfaces, hence in particular Kummer surfaces. The Brauer group of generalised Kummer varieties is studied by Skorobogatov--Zarhin \cite{SZ17}. Suppose we are in a situation where (we do not expect) the Brauer group to obstruct the Hasse principle on $X_\alpha$.  Suppose also that $X_\alpha(\mathbb{A}_k)\neq \emptyset$. From the discussion in Example \ref{ex:Kummer} this implies, in particular, that for each place $v$ of $k$, there is a quadratic character $\chi_v:G_{k_v}\to \mu_2$ such that $Y_\alpha^{\chi_v}(k_v)\neq \emptyset$. The basic strategy (which often turns out to be too optimistic) for establishing the existence of a $k$-point on $X_\alpha$ conditional on finiteness of Shafarevich--Tate groups, goes as follows: 
 \begin{itemize}
...
Let $v$ be a place of $k$. We will denote by $\delta$ both the map $A(k_v)/2A(k_v)\to H^1(k_v,A[2])$ considered previously, and the map $A^\chi(k_v)/2A^\chi(k_v)\to H^1(k_v,A^\chi[2])\cong H^1(k_v,A[2])$ given by composing the connecting map for $A^\chi$ with the isomorphism $A^\chi[2]\cong A[2]$ induced by $\phi^{-1}$. We thus have two self-dual Selmer structures $\{\delta(A(k_v))\}_v$ and $\{\delta(A^\chi(k_v))\}_v$ on $A[2]$. Note that restricting these connecting maps to $A[2]\cong A^\chi[2]$ gives two different connecting maps $\delta:A(k_v)[2]\to H^1(k,A[2])$. The first is the connecting map associated to the short exact sequence  $0\to A[2]\to A[4]\stackrel{2}{\to} A[2]\to 0$. The second is the connecting map associated to the sequence $0\to A[2]\to A^\chi[4]\stackrel{2}{\to} A[2]\to 0$. We denote the second by $\delta_\chi:A(k_v)[2]\to H^1(k,A[2])$.  


\begin{lemma}
For $x\in A(k_v)[2]$ we have $\delta_\chi(x)=\delta(x)+\chi \cup x$. 
\end{lemma}
...
\[\delta(A(k_v))=H^1_{\textup{ur}}(k_v,A[2])=\delta(A^\chi(k_v)).\]
\item[(ii)] If $\chi$ is ramified at $v$ then we have $\delta(A^\chi(k_v))=\delta_\chi(A(k_v)[2])$. Moreover, we have 
\[\delta(A(k_v))\cap \delta^\chi(A(k_v))=0.\]
\end{itemize}
\end{lemma}
...
(ii). Since $A$ has good reduction at $v$, $A[4]$ is unramified. On the other hand, since $\chi$ is ramified at $v$, there is $\sigma$ in the inertia group $I_v$ at $v$ such that $\chi(\sigma)=-1$. Then $\sigma$ acts a multiplication by $-1$ on $A^\chi[4]$. From this, we deduce that $H^0(k_v,A^\chi[4])=A(k_v)[2]$. Thus, $\delta_\chi:A(k^{\textup{ur}}_v)[2]\to H^1(k^{\textup{ur}},A[2])$ is injective. In particular, $\delta_\chi:A(k_v)[2] \to H^1(k_v,A[2])$ is injective. Lemma \ref{basic_conds_lemma} (ii) gives $\#\delta(A^\chi(k_v))=\#A(k_v)[2]$, so we deduce that $\delta(A^\chi(k_v))=\delta_\chi(A(k_v)[2])$. Finally, we have $\delta(A(k_v))=H^1_{\textup{ur}}(k_v,A[2])$ so we must show that $H^1_{\textup{ur}}(k_v,A[2])\cap \delta_\chi(A(k_v)[2])=0$. This follows from the injectivity of $\delta_\chi:A(k^{\textup{ur}}_v)[2]\to H^1(k^{\textup{ur}},A[2])$ noted previously. 
\end{proof}

Let $S$ be a finite set of places of $k$ and let $\chi:G_k\to \mu_2$ be a quadratic character. Write 
\[
V_{S}  = \textup{im}\left(\textup{Sel}_2(A) \stackrel{\oplus_{v\in S}\textup{res}_v}{\longrightarrow} \bigoplus_{v\in S} \frac{\delta(A(k_v))}{\delta(A(k_v))\cap \delta(A^{\chi}(k_v))}\right),
\]
and define $V_{S}^{\chi}$ similarly by interchancing $A$ and $A^\chi$ in the definition.  
...
Suppose that $\delta(A(k_v))=\delta(A^{\chi}(k_v))$ for all $v\notin S$. Then 
...
Moreover, we have 
\[
\dim_{\mathbb{F}_2} V_S  + \dim_{\mathbb{F}_2} V_S^{\chi} \leq \sum_{v \in S} \dim_{\mathbb{F}_2} \frac{\delta(A(k_v))}{\delta(A(k_v)) \cap \delta(A^{\chi}(k_v))}.
\]
 \end{proposition}
...
\[0\to \textup{Sel}_\mathcal{L}(A[2])\to \textup{Sel}_2(A)\stackrel{\oplus_{v\in S}\textup{res}_v}{\to} V_S\to 0\]
and 
\[0\to \textup{Sel}_\mathcal{L}(A[2])\to \textup{Sel}_2(A^\chi)\stackrel{\oplus_{v\in S}\textup{res}_v}{\to} V_S^\chi\to 0.\]
Taking $\mathbb{F}_2$-dimensions gives Equation \eqref{eq:selmer_change_dims}.

For each place $v$, since both $\mathcal{F}_v$ and $\mathcal{L}_v$ are their own orthogonal complements with respect to the local Tate pairing, we have a non-degenerate pairing
...
Further, since the Selmer conditions for $A$ and $A^\chi$ agree for all places $v\notin S$, we have $\textup{inv}_v(x\cup y)=0$ for all $v\notin S$. Thus, we deduce that $V_S$ and $V_S^\chi$ are orthogonal with respect to the pairing \eqref{summed_tate_pairing}. In particular, we have 
...
\begin{cor}
Let $\Sigma$ be a finite set of places of $k$ containing all archimedean places, all places dividing $2$ and all places of bad reduction for $A$. Suppose that $\chi:G_k\to \mu_2$ is a quadratic character that is trivially locally at all places in $\Sigma$. Suppose that $\chi$ is ramified at a single place $v\notin \Sigma$ and that the restriction map $\textup{res}_v:\textup{Sel}_2(A)\to H^1_{\textup{ur}}(k_v,A[2])$ is surjective. Then \[\dim_{\mathbb{F}_2} \textup{Sel}_2(A^{\chi}) = \dim_{\mathbb{F}_2} \textup{Sel}_2(A)  - \dim_{\mathbb{F}_2} A(k_v)[2].\]
\end{cor}
...
%\section{Rational points on Kummer varieties I}

%\subsection{Extensions defined by cocyles}

%Write $G=\textup{Gal}(k(A[2])/k)$. Let $T=\{\alpha_1,...,\alpha_t\}$ be a finite subset of $H^1(k,A[2])$ and represent each $\alpha_i$ by a $1$-cocycle $\tilde\alpha_i:G_k\to A[2]$. 
...
%
%\begin{lemma}
%Suppose that parts (a) and (b) of Condition \ref{cond:simple} are satisfied. Suppose also that $T$ is $\mathbb{F}_2$-linearly indepdendent. Then $\varphi_T:\textup{Gal}(k_T/k)\to A[2]^T\rtimes G$ is an isomorphism. 
%\end{lemma}
%
%\begin{proof}
%It suffices to show that the restriction of $\varphi_T$ to $L=k(A[2])$ induces a surjection $\textup{Gal}(k_T/L)\to A[2]^T$.  Since $H^1(k,A[2])=0$, the inflation-restriction exact sequence shows that $\{\alpha_1|_{L},...,\alpha_t|_L\}$ is a linearly independent subset of $\textup{Hom}_G\big(\textup{Gal}(k_T/L),A[2]\big)$, where $G$ acts on $\textup{Gal}(k_T/L)$ by conjugation. Note in particular that $\dim_{\mathbb{F}_2}\textup{Hom}_G(\textup{Gal}(k_T/L),A[2])\geq t$.
%
...
%
%\begin{proof}
%It suffices to show that the derived subgroup of $A[2]^T\rtimes G$ contains $A[2]^T$. Let $x\in A[2]$ and $\sigma \in G$. Then the commutator of $(0,...,x,0,...,1)$ and $(0,...,0,\sigma)$ is $(0,...,\sigma x-x,0,...,1)$. Since $A[2]$ is a simple $\mathbb{F}_2[G]$-module, it is generated over $\mathbb{F}_2$ by all elements of the form $\sigma x-x$ for $\sigma \in G, x\in A[2]$. 
%\end{proof}
%
...
%
%\section{Rational points on Kummer varieties II}
%
%Incorporation of a second descent step:  potentially some stuff on Harpaz's paper, or my paper on Kummers or both. 
%
%\section{Brief discussion of dp4s?}
% 
%Brief discussion of how (parts of) paper with Alexei works if there's time left over (most likely not?).
