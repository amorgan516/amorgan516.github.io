...
%Let $f(x)\in k[x]$ be a separable polynomial of degree $5$ or $6$, and let $C:y^2=f(x)$ be the associated genus $2$ curve. Let $A$ be the Jacobian of $C$. Denote by $\mathcal{W}$ the $G_k$-set of Weierstrass points on $C$. If $\deg(f)$ is even, then $\mathcal{W}$ is naturally identified with the set of roots of $f(x)$ in $\bar{k}$ (the map given by sending a root $r$ of $f(x)$ to the point $(r,0)\in C(\bar{k})$). If $\deg(f)$ is odd then, as a $G_k$-set, $\mathcal{W}$ is the union of the set of roots of $f(x)$ with the unique point at infinity on $C$. 
%
%As a $G_k$-module, $A[2]\cong \mathbb{F}_2[\mathcal{W}]_{\textup{sum}=0}/\left \langle \sum_{w\in \mathcal{W}}w\right \rangle$. Explicitly, for each pair $P, Q$ of Weierstrass points, the class of the divisor $P-Q$ is $2$-torsion.   One sees from this description that $G=\textup{Gal}(k(A[2])/k)$ is naturally identified with the Galois group of $f(x)$. Both the Weil pairing on $A[2]$ and the $G_k$-set of quadratic refinements of the Weil pairing on $A[2]$ can be described explicitly via this isomorphism. It turns out that the class $c_2\in H^1(G,A[2])$ relevant to Proposition \ref{PS_quad_refinement_prop} is equal to the $A[2]$-torsor $\mathbb{F}_2[\mathcal{W}]_{\textup{sum}=1}/\left \langle \sum_{w\in \mathcal{W}}w\right \rangle$ (where the `quotient' here is the equivalence relation given by adding $\sum_{w\in \mathcal{W}}w$). In fact, there is an explicit description of the (pairing induced by the) Weil pairing on $\mathbb{F}_2[\mathcal{W}]_{\textup{sum}=0}/\left \langle \sum_{w\in \mathcal{W}}w\right \rangle$, and an explicit construction associating a quadratic refinements of the Weil pairing to each odd sized subset of $\mathcal{W}$. The upshot is that if $\deg(f)$ is odd then the class $c_2$ is trivial. 
%
%With some more effort, one can show that if $\textup{Gal}(f)=S_{\textup{deg}(f)}$, then $A[2]$ is a simple $G$-module and $\textup{End}(A[2])=\mathbb{F}_2$. Moreover, in this case we have $H^1(G,A[2])=0$ if $\deg(f)$ is odd, and $H^1(G,A[2])=\left \langle c_2\right \rangle$ if $\deg(f)$ is even. 
%\end{example}
%
