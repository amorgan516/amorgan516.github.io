...
This are notes from a mini-course given as part of the Thematic Programme on Rational Points, Algebraic Cycles and the Local-Global Principle at the Lodha Mathematical Sciences Institute, Mumbai, April 2026. I would like to thank the organisers of that programme, Jean-Louis Colliot-Thélène, Parimala Raman and Federico Scavia, for the opportunity to give the course. I would also like to thank the Lodha institute for the  excellent hospitality and working conditions provided, and everyone who attended the course for their questions and comments throughout. 

\subsection{Notation/background}
For a  field $k$, we will denote by $\bar{k}$ a fixed algebraic closure of $k$, $k^s\subseteq \bar{k}$ the separable closure of $k$ in $\bar{k}$, and $G_k=\textup{Gal}(k^s/k)$ the absolute Galois group of $k$. For a smooth projective variety $X$ over $k$ we write $\textup{Br}(X)$ for the Brauer group of $X$ and set
\[\textup{Br}_1(X)=\ker\big(\textup{Br}(X)\to \textup{Br}(X_{k^s})\big).\] 
We also write $\textup{Br}_0(X)$ for the image of $\textup{Br}(k)$ in $\textup{Br}(X)$. If $H^3(k,k^{s,\times})=0$ (e.g. if $k$ is a number field) then we have an isomorphism $\textup{Br}_1(X)/\textup{Br}_0(X)\cong H^1\big(k,\textup{Pic}(X_{\bar{k}})\big)$; see \cite[Proposition 4.3.2]{ctsko21} for details.
...
Let $\Sha_{\textup{div}}(A)$ denote the maximal divisible subgroup of $\Sha(A)$. Thus, the Cassels--Tate pairing give a non-degenerate pairing on $\Sha_{\textup{nd}}(A):=\Sha(A)/\Sha_{\textup{div}}(A)$. One still does not know unconditionally that $\Sha_{\textup{nd}}(A)$ is finite. However, for all primes $p$, its $p$-part is known to be finite. Corollary  \ref{PS:cor} then holds unconditionally for $\Sha_{\textup{nd}}(A)[2^\infty]$ (meanwhile,  $\Sha_{\textup{nd}}(A)[p^\infty]$ is the square of a finite abelian $p$-group for all odd primes $p$).
\end{remark}
...
Now take $k$ to be a number field.  By a conjecture of Skorobogatov, one expects that the Brauer--Manin obstruction explains all failures of the Hasse principle for $K3$ surfaces, hence in particular Kummer surfaces. The Brauer group of generalised Kummer varieties is studied by Skorobogatov--Zarhin \cite{SZ17}. Suppose we are in a situation where (we do not expect) the Brauer group to obstruct the Hasse principle on $X_\alpha$.  Suppose also that $X_\alpha(\mathbb{A}_k)\neq \emptyset$. From the discussion above this implies, in particular, that for each place $v$ of $k$ there is a quadratic character $\chi_v:G_{k_v}\to \mu_2$ such that $Y_\alpha^{\chi_v}(k_v)\neq \emptyset$. The basic strategy (which often turns out to be too optimistic) for establishing the existence of a $k$-point on $X_\alpha$, conditional on finiteness of Tate--Shafarevich groups, goes as follows: 
 ...
 \end{itemize}
 We will see later some situations where the strategy works in exactly this way. However, there are many others where this strategy runs into difficutly and additional tricks must be found to save it. A key recent innovation, due to Harpaz \cite{Har19}, is to incorporate a second descent step (more about this later). 
 
 

\section{Selmer groups}

We begin with the abstract famework of Selmer structures, before specialising to Selmer groups of abelian varieties. In this section, $k$ denotes a number field.
...
\[\dim_{\mathbb{F}_2}\textup{Sel}_2(A) = \textup{rk}_2(A)+\dim_{\mathbb{F}_2} A(k)[2]+\dim_{\mathbb{F}_2}\Sha_{\textup{nd}}(A)[2].\]
 The first term, $\dim_{\mathbb{F}_2}A(k)[2]$, is constant in the quadratic twist familiy of $A$. 
...
The assumptions on the special prime $\mathfrak{p}$ entail that the group of geometric connected components of the special fibre of the N\'{e}ron model of $A$ over $k_\mathfrak{p}$ is trivial and that $\mathfrak{f}(A/k_v)=1$ (cf. \cite[Proof of Corollary 2.4]{HS16}). Let $\psi_\mathfrak{p}$ be the unique non-trivial unramified quadratic character. Since $\alpha$ is unramified at $\mathfrak{p}$, it follows from Proposition \ref{prop:Neron_model}(ii) that $\alpha$ lies in both $\textup{im}(\delta_v)$ and $\textup{im}(\delta_v^{\psi_{\mathfrak{p}}})$. Since $\mathfrak{f}(A/k_v)$ is odd, by Proposition \ref{prop:Neron_model}(i), Theorem \ref{Thm:parity} and Remark \ref{rem:Selmer_parity} we can,  choosing $\chi_{\mathfrak{p}}$ to be either the trivial character or non-trivial unramified character as appropriate,  ensure that $\dim \textup{Sel}_2(A^\chi)$ is odd for any global character $\chi:G_k\to \mu_2$ such that $\textup{res}_v(\chi)=\chi_v$ for all $v\in \Sigma$. For any such $\chi$, we further have $\textup{res}_{v}(\alpha)\in \textup{im}(\delta_v^{\chi})$ for each $v\in \Sigma$.
...
Let $\chi_0:G_k\to \mu_2$ be a character afforded by Lemma \ref{approximating_extensions_lemma}, so that  $\alpha \in \textup{Sel}_2(A^{\chi_0})$ and $\dim \textup{Sel}_2(A^{\chi_0})\equiv 1 ~(\textup{mod }2)$.  
...
\item instead of twisting to shrink the $2$-Selmer group, use the Mazur--Rubin machinery to produce many quadratic twists all having odd $2^\infty$-Selmer rank and identical $2$-Selmer group to that of $A^{\chi_0}$. (This step uses the existence of elements $\sigma \in \textup{Gal}(f)$ such that $A[2]^\sigma=0$, e.g. $5$-cycles. The key point is that, if $\mathfrak{q}$ is a prime such that $A[2]^{\textup{Frob}_\mathfrak{q}}=0$ then $H^1(k_\mathfrak{q},A[2])=0$, hence the Selmer conditions at $\mathfrak{q}$ are vacuous.)
...
% (2023).  
   ...
   \bibitem[Fla90]{Fla90}  M. Flach. \textit{A generalisation of the Cassel-Tate pairing}, Journal für die reine und angewandte Mathematik 412 (1990), 113-127.
