...
Let $T=\{\alpha\}$ and let $\widetilde{\alpha}$ be a $1$-cocycle representing $\alpha$. Lemma \ref{hyperelliptic_twisting_nonsense} and Lemma \ref{semidirect_product_lem} show that the map $\varphi_T:\textup{Gal}(k_T/k)\to A[2]\rtimes \textup{Gal}(f)$, given by $\sigma \mapsto (\widetilde{\alpha},\bar{\sigma})$, is an isomorphism. We also deduce from that lemma that the maximal abelian subextension of $k_T/k$ is $k(\sqrt{\textup{disc}(f)})$. Let $\mathcal{C}\subseteq \textup{Gal}(k_T/k)$ be the set of elements conjugate to an element of $\varphi_T^{-1}(0\times \textup{Gal}(f))$. Since this contains an element acting on the roots of $f(x)$ as an odd permutation, the set $\mathcal{A}(\mathcal{C})$ in Lemma \ref{approximating_extensions_lemma} is trivial. We deduce the existence of a global quadratic character $\chi$ with $\textup{res}_v(\chi)=\chi_v$ for all $v\in \Sigma$ and such that, outside $\Sigma$, $\chi$ is ramified only at primes $\mathfrak{q}$ for which $\textup{Frob}_{\mathfrak{q}}\in \mathcal{C}$. By construction, $\textup{res}_{\mathfrak{q}}(\alpha)=0$ for all such primes. This, combined with the choice of $\{\chi_v\}_{v\in \Sigma}$, ensures that $\alpha\in \textup{Sel}_2(A^\chi)$, completing the proof.
 ...
 Without loss of generality, we have $\alpha \neq 0$ (else $X_\alpha$ trivially has a $k$-point). Further, we can assume $X_{\alpha}(\mathbb{A}_k)\neq \emptyset$ (else  $X_\alpha$ trivialy satisfies the Hasse principle). For simplicity we will assume that finiteness of Shafarevich--Tate groups throughout. 
 
 Since $\deg(f)=5$, Lemma \ref{hyperelliptic_twisting_nonsense} ensures that $c_2$ is zero. Thus, having assumed finiteness of Shafarevich--Tate groups, we have $\dim \Sha(A^{\chi})[2]\equiv 0 \pmod 2$ for all quadratic characters $\chi$ (cf. Proposition \ref{PS_quad_refinement_prop}). One consequence of this is that $\textup{rk}(A^\chi)\equiv \dim\textup{Sel}_2(A^\chi) ~(\textup{mod }2)$ for all $\chi$ (thanks to the exact sequence  \ref{eq: selmer_descent}). 
 
 We first show the existence of a quadratic character $\chi_0:G_k\to \mu_2$ such that $\alpha \in \textup{Sel}_2(A^{\chi_0})$ and $\dim \textup{Sel}_2(A^{\chi_0})\equiv 0 (\textup{ mod }2)$. The assumption that $X_\alpha(\mathbb{A}_k)\neq \emptyset$ 
